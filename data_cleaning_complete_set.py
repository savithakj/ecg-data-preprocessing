...
def rejection_condition_two(complete_data):
    '''
...
def rejection_condition_one(complete_data):
    '''
    ...
    for row in complete_data:
        ...
        for index in peaks:
            if round(float(row[index + 1])) == 0:
                ...
                flag = True
        if flag == True:
            incorrect_records_list.append(record_id)
            flag = False
        else:
            correct_records.append(record_id)
        record_id = record_id + 1
    return incorrect_records_list, correct_records
